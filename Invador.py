...
# Set this to the GPIO you wired the piezo + lead to.
BUZZER_PIN = board.GP15
# Allow frequency changes safely
buzzer = pwmio.PWMOut(BUZZER_PIN, frequency=4000, duty_cycle=0, variable_frequency=True)
beep_until = 0.0

# Murata PKM13EPYH4000-A0 resonates at ~4kHz; 50% duty is fine.
def start_beep(freq=4000, duration=0.08):
    global beep_until
    try:
        buzzer.duty_cycle = 0
...
# Quick hardware test: hold START > 0.5s to play a 0.5s 4kHz tone
start_press_time = None

def update_start_test():
    global start_press_time
    now = time.monotonic()
    if not buttons["start"].value:  # pressed
        if start_press_time is None:
            start_press_time = now
        elif (now - start_press_time) > 0.5:
            start_beep(freq=4000, duration=0.5)
            start_press_time = None  # single-shot per long press
    else:
        start_press_time = None
...
while True:
    ...
    if fire_now and not fire_prev:
        ...
        start_beep(freq=4000, duration=0.12)  # <- only change: longer beep perceived louder
    fire_prev = fire_now
